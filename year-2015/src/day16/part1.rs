...
fn main() {
    utils::inputs_str!("1").split('\n').map(Aunt::from)
        .filter(|aunt| aunt.children == OUR_SUE.children || aunt.children.is_none())
        .filter(|aunt| aunt.cats == OUR_SUE.cats || aunt.cats.is_none())
        .filter(|aunt| aunt.samoyeds == OUR_SUE.samoyeds || aunt.samoyeds.is_none())
        .filter(|aunt| aunt.pomeranians == OUR_SUE.pomeranians || aunt.pomeranians.is_none())
        .filter(|aunt| aunt.akitas == OUR_SUE.akitas || aunt.akitas.is_none())
        .filter(|aunt| aunt.vizslas == OUR_SUE.vizslas || aunt.vizslas.is_none())
        .filter(|aunt| aunt.goldfish == OUR_SUE.goldfish || aunt.goldfish.is_none())
        .filter(|aunt| aunt.trees == OUR_SUE.trees || aunt.trees.is_none())
        .filter(|aunt| aunt.cars == OUR_SUE.cars || aunt.cars.is_none())
        .filter(|aunt| aunt.perfumes == OUR_SUE.perfumes || aunt.perfumes.is_none())
